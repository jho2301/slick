...
const answerFor = (
  provider: string,
  model: string,
  extra: Partial<ProfileModelAnswer> = {}
): ProfileModelAnswer => ({
  providers: CATALOG,
...
type Read = Deferred<ProfileModelAnswer> & { name: string };
type Write = Deferred<ProfileModelAnswer> & { name: string; wanted: unknown };
type UsageRead = Deferred<UsageAnswer> & { name: string; opts: { refresh?: boolean } };

interface FakeApi extends HermesApi {
  reads: Read[];
  writes: Write[];
  profileCalls: Deferred<HermesProfileSummary[]>[];
  usageReads: UsageRead[];
}
...
function fakeApi({
  profiles = [
    { name: 'work', isDefault: true, configured: true },
    { name: 'lab', isDefault: false, configured: true },
  ],
}: { profiles?: HermesProfileSummary[] } = {}): FakeApi {
  const reads: Read[] = [];
  const writes: Write[] = [];
  const profileCalls: Deferred<HermesProfileSummary[]>[] = [];
  const usageReads: UsageRead[] = [];
  return {
    reads,
    writes,
    profileCalls,
    usageReads,
    hermesProfiles() {
      const pending = deferred<HermesProfileSummary[]>();
    ...
    hermesProfileModel(name) {
      const pending = { name, ...deferred<ProfileModelAnswer>() };
      reads.push(pending);
      return pending.promise;
    },
    setHermesProfileModel(name, wanted) {
      const pending = { name, wanted, ...deferred<ProfileModelAnswer>() };
      writes.push(pending);
      return pending.promise;
    },
    hermesProfileUsage(name, opts = {}) {
      ...
      const pending = { name, opts, ...deferred<UsageAnswer>() };
...
describe('reading a profile', () => {
  test('the first load asks for the first profile and shows what it says', async () => {
    ...
    assert.ok(renders > 0, 'every change was announced');
  });
  ...
  test('an out-of-order answer for a profile nobody is on any more is ignored', async () => {
    await loaded();
    void store.selectProfile('lab');
    await settle();
    void store.selectProfile('work');
    await settle();
...
describe('saving', () => {
  ...
  test('switching profile mid-save abandons the answer instead of applying it', async () => {
    ...
    assert.deepEqual(
      store.state.saved,
      { provider: 'openai-codex', model: 'gpt-6-vega' },
      "work's readback never lands on lab"
    );
  ...
  test('a refused write keeps the draft and says what happened', async () => {
    ...
    assert.deepEqual(
      ...
      'the known default is still known'
    );
    assert.deepEqual(
      store.state.draft,
      { provider: 'anthropic', model: 'claude-opus-5' },
      'the choice is not thrown away'
    );
...
describe('a read that failed', () => {
  ...
  test('a failed refresh keeps the default it already knows on screen', async () => {
    ...
    assert.deepEqual(
      store.state.saved,
      { provider: 'openai-codex', model: 'gpt-6-astra' },
      'the last known default is still shown'
    );
...
describe('the level the profile thinks at', () => {
  ...
  test('going back to the Hermes default asks for the setting to be cleared', async () => {
    ...
    await settle();
    assert.equal(
      (api.writes[0]!.wanted as { effort: string }).effort,
      '',
      'an empty level is a request to unset it, not a missing field'
    );
  ...
  test('the level cannot be moved while the panel is busy', async () => {
    ...
    store.setEffort('max');
    assert.equal(
      store.state.draftEffort,
      'high',
      'a draft moved mid-request could be written to another profile'
    );
  });
...
describe('what the account behind the profile has left', () => {
  ...
  const window = (label: string, used: number, resetAt: string | null = null) => ({
    label,
    usedPercent: used,
  ...
  const usageAnswer = (extra: Partial<UsageAnswer> = {}): UsageAnswer => ({
    profile: 'work',
    usage: {
      provider: 'openai-codex',
      supported: true,
  ...
  async function onCodex(answer: UsageAnswer | null = usageAnswer()) {
    void store.load();
    await settle();
    api.reads.at(-1)!.resolve(answerFor('openai-codex', 'gpt-6-astra'));
    await settle();
  ...
  test('a second click while one is out is not a second request', async () => {
    ...
    await settle();
    assert.equal(
      api.usageReads.length,
      2,
      'one ask outstanding at a time — the rate limit is the provider’s'
    );
  });
