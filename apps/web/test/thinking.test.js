...
import { applyChunk, emptyThink, readThinking, settle, stepStatusLabel, THINK_KEY } from '../js/thinking.js';
...
test('a step never moves', () => {
  ...
  applyChunk(think, { s: [{ id: 'b', st: 'complete' }, { id: 'c', t: 'Writing…', st: 'in_progress' }] });
...
test('a source with no url is not a link', () => {
  const think = readThinking(
    reply({ [THINK_KEY]: { s: [{ id: 't1', t: 'Searched', src: [{ t: 'nowhere' }, { u: 'https://x.test/' }] }] } })
  );
...
test('the browser and the server fold the same patches the same way', () => {
  ...
  const sourced = agree(
    [
      { s: [{ id: 't2', t: 'Compared the two images', st: 'complete', src: [{ u: 'https://example.test/91', t: 'build 91' }] }] },
    ],
