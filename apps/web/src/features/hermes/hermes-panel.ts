 ...
 *
 * No DOM, no fetch. The panel draws what these functions decide, which is
 * what lets the awkward cases — a provider whose credentials were revoked, a
 * model retired upstream, a provider with no models at all — be tested as the
 * small facts they are.
 *
 * Nothing here is about a *session*. `_serveModel` on an agent session is one
 * conversation's override and lives in `model-picker.ts`, where a model id is
...
const listOf = <T>(value: readonly T[] | null | undefined): readonly T[] => value ?? [];

/** The pair the two selects show, or the pair a profile is on. */
export interface Selection {
  provider: string;
  model: string;
}

/** A pair as the wire, a form, or a test may spell it: `null` and `''` are one absence. */
...
function modelsOf(
  providers: readonly HermesProviderEntry[] | null | undefined,
  provider: string
): HermesModelEntry[] {
 ...
 */
export function withConfigured(
  providers: readonly HermesProviderEntry[] | null | undefined,
  { provider, model }: SelectionInput = {}
): HermesProviderEntry[] {
  ...
  return entries.map((entry) =>
    entry === found
      ? {
          ...entry,
          models: [...listOf(entry.models), { value: wantedModel, label: wantedModel, unlisted: true }],
        }
      : entry
  );
}
 ...
 */
export function hermesSelection(
  providers: readonly HermesProviderEntry[] | null | undefined,
  { provider, model }: SelectionInput = {}
): Selection {
  const entries = listOf(providers);
  ...
  if (!chosen) return { provider: wantedProvider, model: wantedModel };
...
export function sameSelection(
  a: SelectionInput | null | undefined,
  b: SelectionInput | null | undefined
): boolean {
 ...
 */
export function hermesEfforts(
  efforts: readonly EffortChoice[] | null | undefined,
  configured: string | null | undefined
): EffortOption[] {
  const options: EffortOption[] = [
    { value: '', label: 'Hermes default' },
    ...listOf(efforts).map((e) => ({ value: e.value, label: e.label })),
  ];
 ...
 */
export function sameEffort(a: unknown, b: unknown): boolean {
...
export function hasAccountLimits(provider: unknown): boolean {
 ...
 */
export function shouldRefreshUsageAfter(
  message: { author?: { kind?: string } | null } | null | undefined,
  provider: unknown
): boolean {
...
/** A usage payload as it may arrive: the daemon's shape, or any part of it. */
export type UsageInput = Partial<Omit<AccountUsage, 'windows'>> & { windows?: Partial<UsageWindow>[] | null };

export interface UsageRow {
  label: string;
  used: number | null;
  remaining: number | null;
  /** What a meter would fill. An unknown window is an empty bar, not a full one. */
  fill: number;
  known: boolean;
  resetAt: string | null;
  detail: string | null;
 ...
 */
function usageRow(window: Partial<UsageWindow> | null | undefined): UsageRow {
 ...
 */
export function usageWindows(usage: UsageInput | null | undefined): UsageRow[] {
 ...
 * missing.
 */
const LIMIT_ROWS: readonly { key: LimitRow['key']; label: string; matches: (name: string) => boolean }[] = [
...
export function usageLimitRows(usage: UsageInput | null | undefined): LimitRow[] {
  ...
  return LIMIT_ROWS.map(({ key, label, matches }) => {
    ...
    return {
      ...
      // Derived from `used` when the provider only reported the spent half;
      // null stays null, because "not reported" must not arrive at the row as
      // "100% left".
...
export function resetStamp(iso: unknown): string | null {
 ...
 *
 * @param now  epoch ms, passed in so this is testable without a clock
 */
export function resetPhrase(iso: unknown, now: number = Date.now()): string | null {
  const at = Date.parse(asText(iso));
 ...
 */
export function resetWithin(iso: unknown, now: number = Date.now()): string | null {
  const at = Date.parse(asText(iso));
  if (!Number.isFinite(at)) return null;
  const seconds = Math.round((at - now) / 1000);
  if (seconds <= 0) return 'now';
 ...
 */
export function usageLimitText(row: Partial<LimitRow> | null | undefined, now: number = Date.now()): string {
...
export type UsageStatusKind = 'unsupported' | 'signed-out' | 'error' | 'empty' | 'ok';
...
/** An answer as it may arrive: the daemon's shape, or any part of it. */
export type UsageAnswerInput = Partial<Omit<UsageAnswer, 'usage'>> & { usage?: UsageInput | null };
 ...
 */
export function usageStatus(answer: UsageAnswerInput | null | undefined): UsageStatus {
...
export function usageAge(
  answer: { fetchedAt?: string | null; throttled?: boolean } | null | undefined,
  now: number = Date.now()
): string | null {
 ...
 */
export function bankedResetLine(count: unknown): string | null {
...
export function usageDetailLines(details: unknown): string[] {
  return (Array.isArray(details) ? details : []).filter(
    (line): line is string => Boolean(asText(line)) && !BANKED_DETAIL.test(String(line))
  );
}
