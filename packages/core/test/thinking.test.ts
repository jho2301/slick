 ...
 *
 * Almost all of this is pure — that is the point of `thinking.ts` having no
...
import { THINK_KEY, normalizeThinking, mergeThinking, type ThinkingTrace } from '../src/index.ts';
import { Workspace } from '../src/workspace.ts';

const stepIds = (think: ThinkingTrace | null) => think?.s.map((s) => s.id);
const bytes = (think: unknown) => Buffer.byteLength(JSON.stringify(think), 'utf8');
...
describe('mergeThinking upserts by id', () => {
  ...
  test('the caps hold across a merge, not just a normalize', () => {
    let think: ThinkingTrace | null = null;
    for (let i = 0; i < 80; i++) {
...
describe('a blob on a message', () => {
  ...
  test('an oversized blob is clamped on the way in', () => {
    ...
    const stored = ws.messages.get(posted.id).metadata![THINK_KEY] as ThinkingTrace;
