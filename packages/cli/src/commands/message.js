 ...
 */
async function splitChannelAndWords(ctx, rest) {
  if (!ctx.flags.thread) {
    return { channel: requireRef(rest[0], 'channel'), words: rest.slice(1) };
  }
  if (rest.length > 1 && (await ctx.ws.channels.find(rest[0]))) {
...
async function postMessage(ctx, { channel, parentId, words }) {
  const text = await resolveText(words);
  const author = ctx.flags.as
    ? { id: String(ctx.flags.as).replace(/^@/, ''), kind: ctx.flags.bot ? 'agent' : 'human', label: ctx.flags.as }
    : undefined;
  return ctx.ws.messages.post({
    channel,
    parentId: parentId ?? null,
    text,
    author,
    metadata: parseJsonFlag(ctx.flags.meta, 'meta') ?? null,
  });
}
...
Options
  ...
  async run(ctx) {
    ...
    switch (sub) {
      ...
      case 'list':
      case 'read': {
        const result = await ctx.ws.messages.list(requireRef(rest[0], 'channel'), {
          limit: ctx.flags.limit ? Number(ctx.flags.limit) : 30,
          before: ctx.flags.before,
          after: ctx.flags.after,
          includeReplies: Boolean(ctx.flags.replies),
          includeDeleted: Boolean(ctx.flags.deleted),
        });
      ...
      case 'show': {
        const found = await ctx.ws.messages.get(requireRef(rest[0], 'message id'));
        ...
        if (found.replyCount > 0) {
          line();
          note(`  ${found.replyCount} repl${found.replyCount === 1 ? 'y' : 'ies'} — slick thread show ${found.id}`);
        }
        return;
      }
      ...
      case 'edit': {
        ...
        const updated = await ctx.ws.messages.update(id, {
          text: await resolveText(rest.slice(1)),
          metadata: parseJsonFlag(ctx.flags.meta, 'meta'),
        });
      ...
      case 'delete':
      case 'rm': {
        const removed = await ctx.ws.messages.remove(requireRef(rest[0], 'message id'), {
          hard: Boolean(ctx.flags.hard),
        });
        if (ctx.json) return json({ message: removed });
        ok(
          ctx.flags.hard
            ? `Deleted ${removed.id} permanently` +
                (removed.deletedReplies ? ` along with ${removed.deletedReplies} repl${removed.deletedReplies === 1 ? 'y' : 'ies'}.` : '.')
            : `Deleted ${removed.id}.`
        );
        return;
      }
...
Options
  ...
  async run(ctx) {
    ...
    switch (sub) {
      case 'show':
      case 'open': {
        const result = await ctx.ws.messages.thread(requireRef(rest[0], 'message id'));
        if (ctx.json) return json(result);
